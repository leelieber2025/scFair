...
smaller types often miss the cut. Default `append` keeps that ranking and
adds a short **same-rank** tail of near-miss genes so the base top-`k` is never
displaced. Use `balance_method="none"` for pure top-`k`.

See {doc}`user_guide/method` for details.

## Is `append` different from just taking a larger `k`?

For the gene set, no.
`balance_method="append"` with base `k` and budget `m` selects the same genes
as `balance_method="none"` with `n_top_genes=k+m`. Auto chooses `k`, then
append adds a fixed-style tail. Metadata records base vs tail.
...
If you already stored full counts in `adata.raw` (the usual scanpy pattern)
and did not pass `store_raw=True`, `restore_raw_counts(..., full_genes=True)`
uses that `.raw` matrix.

## Do I need to cluster first?

No. After auto chooses a base size, `append` is one global ranking plus a
small extension. Gene selection does not run Leiden.
...
list). When you turn them on, scFair detects human vs mouse naming from
`var_names` (Ensembl `ENSG` / `ENSMUSG`, mito `MT-` vs `mt-`, ribo casing),
then drops matching mitochondrial symbols and ribosomal structural proteins
(`RPL*` / `RPS*` / `Rpl*` / `Rps*`; not kinases like `RPS6KA*`) and refills
from the global ranking. Forced `marker_genes` are never removed.
...
`"force"`: those genes are present in the final output regardless of where
they rank in the global HVG pass.
...
selected, and the gene list is unchanged:
...
`n_top_genes` selection. If you set `options=HVGOptions(marker_extra=False)`,
markers are folded *into* the `n_top_genes` budget and may displace the
lowest-ranked algorithm-selected genes.
...
`layers["counts"]` exist but disagree, scFair will not overwrite your
layer; pass `layer=` explicitly.

## Does scFair keep a raw counts matrix in `uns`?

No, not by default. A second full matrix under
`uns["scfair"]["raw_snapshot"]` is written only when you pass
`options=HVGOptions(store_raw=True)` (or `"ondisk"`). Use that when you call
`subset=True` and later need the pre-subset gene universe.
...
`batch_key` is on {class}`scfair.pp.HVGOptions`, not the short public
signature. Point it at a column in `adata.obs`:
...
This is not full batch correction (no Harmony / scVI / BBKNN-style
integration). Default `"append"` uses the same batch-aware ranking for base
top-`k` and the tail (`top-(k+m)`).
...
or set `strict=True` to raise instead of falling back. Tiny gene panels can
also trigger a fallback before loess crashes.

## How do I set `append_budget` or `batch_key`?

Secondary knobs go on {class}`scfair.pp.HVGOptions`, not as top-level
arguments:
...
scf.pp.highly_variable_genes(
    adata,
    n_top_genes=2000,
    options=HVGOptions(append_budget=100, batch_key="batch"),
)
```

## `inplace=False` and `subset=True` together

`inplace=False` returns a DataFrame of gene annotations. It does not return
a new AnnData, so `subset=True` has nowhere to land and is ignored (a
warning is emitted). To subset genes, use `inplace=True, subset=True`, or
apply `adata.var["highly_variable"]` yourself.
...
Auto builds multi-seed HVG → PCA → neighbors → density graphs to estimate
list size. That is much slower than one scanpy HVG pass (often about
10–150×). Progress messages print on stderr once `n_obs >= 1000` under auto.
...
Requires a neighbor graph (or pass `embedding=`). On well-separated data it
tracks the true count well up to about **20 populations**; with many tiny
groups (for example 30 types × tens of cells each) it can under-count. It
answers *how many*, not *which cell goes where*.

## How does this differ from GiniClust or CellSIUS?

GiniClust / CellSIUS hunt **rare-subpopulation markers** (Gini or per-cluster
tests, usually after a coarse partition). Default scFair **`append`** only
extends the **same global HVG ranking** by a short tail (`k+1 … k+m`). No
Gini LOESS pool, no cluster-wise FDR, no co-expression communities.

If rare types are still poorly resolved, pass known markers with
...
and (by default) appends near-miss genes.
...
Pin the version in analysis code and manuscripts, for example `scfair==0.9.0`.
